...
V2.5 of the sectioning pipeline. Changes from v2.4:

1. TIE-LIKE SLUR DETECTION. Slurs spanning only same-pitch notes are
   notationally slurs but musically ties — they should never have a
   breath mark inside or at their endpoints. We detect these and:
     - Still mark in_slur_interior (peaks inside get suppressed as before)
     - Do NOT contribute to slur-endpoint bumps (no boost at their ends)
     - Pass spans to LBDM so merge_tied_notes treats them as ties,
       collapsing the spanned notes into one event for interval profiles.

2. RESULT PERSISTENCE. New top-level functions save_result(result, path)
   and load_result(path) for caching results to disk. Strips unpicklable
   parts (music21 score, music21 note element references) and adds
   measure_offsets so explain() doesn't need the score object.
   Use cache when iterating with explain on the same piece.

3. EXPLAIN NO LONGER REQUIRES SCORE. Reads measure_offsets directly from
   the result dict. The result returned by run_sectioning still includes
   'score' for callers that want it (e.g., re-annotation), but explain()
   ignores it.

Usage with caching:
    from sectioning_v2_5 import run_sectioning, explain, save_result, load_result
    import os
...
def extract_slur_indicators(part, num_frames, division,
    ...
    Extract slur info for a part. Returns a dict with:
      ...
      in_slur_interior:  shape (num_frames,), 1 strictly inside any slur
                         (phrase OR tie-like). Suppression applies to
                         both kinds.
      endpoint_bumps:    shape (num_frames,), Gaussian-smoothed indicator
                         of phrase-slur endpoints ONLY. Tie-like slurs do
                         not contribute endpoint bumps because they're
                         musically a single sustained note.
      n_slurs:           total number of slurs found.
      n_phrase_slurs:    number of phrase slurs (multi-pitch).
      n_tie_like_slurs:  number of tie-like slurs (same-pitch).
      slur_starts:       onset times of phrase-slur start notes.
      slur_ends:         onset times of phrase-slur end notes.
      tie_like_spans:    list of (start_off, end_off) for same-pitch slurs.
                         Passed to LBDM so merge_tied_notes can collapse
                         the spanned notes into single events.

    NEW IN v2.5: Tie-like slur detection. A slur whose spanned notes all
    share the same pitch (start pitch == end pitch == any intermediate
    pitches) is notationally a slur but musically a tie. Real musicians
    don't breathe between same-pitch slurred notes — it's one sustained
    note. We detect these by walking the slur's spanned elements and
    comparing pitches.

    For chords, the top pitch is used (matching LBDM's chord reduction).
    """
    ...
    for slur in slurs:
        try:
            ...
            end_elem = spanned[-1]
            # CRITICAL: spanned elements live inside measures; their
            # element.offset is measure-local, not global. We need the
            # offset within the part hierarchy for correct alignment to
            # the global time grid.
            try:
                start_off = float(start_elem.getOffsetInHierarchy(part))
                end_off = float(end_elem.getOffsetInHierarchy(part))
            except Exception:
                # Fallback to raw offset if hierarchy lookup fails.
                start_off = float(start_elem.offset)
                end_off = float(end_elem.offset)
            if end_off <= start_off:
                continue

            # Tie-like detection: are all spanned notes the same pitch?
            is_tie_like = False
            try:
                pitches = []
                for el in spanned:
                    if isinstance(el, m21.note.Note):
                        ...
                        pitches.append(None)
                if (len(pitches) >= 2 and
                        all(p is not None for p in pitches) and
            ...
            # Both kinds suppress interior peaks.
            ...
            if is_tie_like:
                ...
                # Tie-like: NO endpoint bumps, NO entries in slur_starts/ends.
                # This prevents boundary detection from firing at the seam
                # between same-pitch slurred notes.
            else:
                n_phrase_slurs += 1
                ...
                if 0 <= start_idx < num_frames:
                    endpoints_raw[start_idx] = max(
                        endpoints_raw[start_idx], 1.0
                    )
                if 0 <= end_idx < num_frames:
                    endpoints_raw[end_idx] = max(
                        endpoints_raw[end_idx], 1.0
                    )
        except Exception:
            continue
...
def run_sectioning(input_path, output_dir='./output',
    ...
    for i in range(len(parts)):
        if is_tacet[i]:
            ...
            continue
        # Pass tie_like_spans so LBDM merges same-pitch-slur chains as ties.
    ...
    # Pre-compute measure_offsets so explain() doesn't need the score.
    measure_offsets = []
    if len(parts) > 0:
    ...
    # Auto-cache to the output directory so explain.py can find it.
