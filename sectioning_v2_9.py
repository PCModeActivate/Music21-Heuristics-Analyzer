...
V2.9 of the sectioning pipeline. Changes from v2.8:

1. Typed structural-event features.
   Dynamics, metric/tempo changes, key changes, time changes, and tempo or
   expression text such as rit., rall., accel., stringendo, meno mosso, plus
   mosso, and a tempo are now represented with typed event kinds and typed
   weights before being projected to the structural bump grid.  This keeps
   the useful "new forte/new tempo/new meter means likely new phrase or
   section" cue, while making it possible to ablate or tune those event
   classes independently.

2. Phrase-start structural annotation.
   A boundary that lands on a structural phrase-start cue, such as a new
   dynamic or tempo/text marking at the onset of a note, is annotated on the
   previous note unless the target note is also an effective slur ending.
   This matches breath-mark semantics: the written breath occurs after the
   previous phrase-ending note, before the new phrase-start event.

3. Tempo-aware short-phrase filtering.
   V2.9 computes an approximate tempo map and can remove extremely short
   phrase fragments after candidate selection.  This is intentionally
   conservative and configurable: a phrase is suspicious only when it is
   short in seconds and contains very few note events.  The default is meant
   to catch one-note/tiny fragments without making "quarter note bad" a
   universal rule.

4. Feature-first ablation support.
   The new structural weights and short-phrase filter parameters are exposed
   through run_sectioning() and the CLI so they can be switched off or tuned
   one feature at a time before building a regression/evaluation layer.

5. V2.8 behavior is otherwise preserved.
   Natural ties and same-pitch tie-like slurs remain hard sustained-event
   no-breath zones.  Slurs, hairpins, and tuplets remain continuation
   penalties rather than universal hard rejects.

Usage with caching:
    from sectioning_v2_8 import run_sectioning, explain, save_result, load_result
    import os
...
def postprocess_short_phrases(boundaries, notes_list, selection_score,
                              ...
                              start_time_qn=0.0):
    """
    Conservatively remove tiny one-note phrase fragments.

    This is not "quarter note bad."  A segment is suspicious only if it is
    short in real seconds AND contains very few note events.  When a tiny
    phrase is found between two boundaries, remove the weaker neighboring
    boundary by selection_score.  The pass repeats until stable.
    ...
    while changed and bdys:
        ...
        for idx, cur in enumerate(list(bdys)):
            seconds = qn_span_to_seconds(prev, cur, tempo_segments)
            note_count = count_note_events_between(notes_list, prev, cur)
            if seconds < min_phrase_seconds and note_count <= max_phrase_notes:
                if idx == 0:
                    remove_idx = idx
                else:
                ...
                removed.append({
                    ...
                    'phrase_seconds': float(seconds),
                    'phrase_note_count': int(note_count),
                })
                changed = True
                break
            prev = cur
...
def add_breath_marks_per_part(original_score, boundaries_per_part,
    ...
    for i, part in enumerate(parts):
        ...
        if natural_tie_info_per_part is not None and i < len(natural_tie_info_per_part):
            natural_tie_spans = natural_tie_info_per_part[i].get('natural_tie_spans', [])
        structural_offsets = []
        structural_kinds = []
        if structural_info_per_part is not None and i < len(structural_info_per_part):
            structural_offsets = structural_info_per_part[i].get('offsets', [])
            structural_kinds = structural_info_per_part[i].get('event_kinds', [])
        ...
        for bt in boundaries:
            ...
            if primary is not None:
                ...
                is_structural_start = any(
                    abs(primary_off - off) <= onset_tolerance and
                    kind in structural_phrase_start_kinds
...
def write_summary_per_part(boundaries_per_part, part_names, activity_levels,
    ...
    summary = {
        'parameters': {
            ...
            'boundary_detection': 'candidate_selection_v2_9',
...
def main():
    parser = argparse.ArgumentParser(
        description="V2.9 sectioning pipeline with chroma/rhythm self-SSM, "
