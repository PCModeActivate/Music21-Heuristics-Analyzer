...
V2.7 of the sectioning pipeline. Changes from v2.6:

1. Separate chroma self-SSM and rhythm self-SSM.
   V2.6 used one per-part SSM over normalized chroma/rest frames. V2.7
   keeps that chroma novelty stream but adds a separate rhythm novelty
   stream over attack/sustain/rest frames. This lets phrase detection
   react to rhythmic repetition or rhythmic discontinuity without treating
   pitch-class changes as rhythmic changes.

2. Cross-part rhythm support is used as soft evidence.
   For each part/time, V2.7 compares local rhythm windows against other
   active parts. If another part has a sufficiently similar rhythmic
   window, that part's nearby local boundary evidence can support the
   current part. This is meant to keep rhythmically doubled lines such as
   Alto Saxophone and Euphonium from choosing different nearby boundary
   peaks when they are playing essentially the same rhythmic gesture.

3. Candidate selection is no longer plain find_peaks on B_i alone.
   V2.7 first collects candidate peaks, then chooses within each local
   exclusion window using a phrase-oriented score:
       B_i + LBDM support + long-note-arrival support + cross-rhythm support
   This avoids selecting a slightly taller texture/SSM spike when a nearby
   long-note arrival is the more plausible breath/phrase endpoint.

4. Cross-part synchronous similarity diagnostics are computed but not used
   for boundary decisions.
   The result dict now stores rhythm_sync and chroma_sync_musical matrices
   of shape (parts, parts, frames). These are time-aligned cross-part
   similarities, not full T-by-T cross-similarity matrices. They are kept
   for inspection and later experiments. The musical chroma diagnostic uses
   an interval-class similarity kernel, where unisons/octaves, thirds,
   sixths, fourths, and fifths can be treated as more related than
   semitone/tritone clashes.

5. Long marks use [start, end) breath semantics.
   A breath mark means "breathe after this note." Therefore the beginning
   note of a slur, tie-like slur, hairpin, or tuplet group is protected,
   because a breath after that beginning note would break the marked span.
   The final note of the span is not protected by the span itself, because
   a breath after the final note is musically allowed.

6. Tuplet spans are now explicit suppression context.
   V2.6 only suppressed LBDM on tuplet-interior notes. V2.7 also builds a
   [start, end) tuplet mask so boundary selection does not place breaths
   after the beginning/interior notes of a tuplet group.

Usage with caching:
    from sectioning_v2_7 import run_sectioning, explain, save_result, load_result
    import os
...
def build_rhythm_feature_vectors(activity, onset,
    ...
    V2.7 intentionally keeps this independent from pitch. Two parts that
...
def compute_cross_part_synchronous_similarity(phi, is_tacet=None,
    ...
    would be much larger. V2.7 stores these diagnostics but does not use
...
def extract_lbdm_profiles(part, tie_like_spans=None):
    """
    Build per-note records for LBDM, with tie-merged events and
    tuplet-interior flagging.

    NEW IN v2.5: tie_like_spans is an optional list of (start_off, end_off)
    tuples representing same-pitch slurs that should be treated as ties.
    For each note, we check if it falls inside any such span and synthesize
    a tie_type accordingly. The first note in a span gets 'start' (if no
    natural tie already), interior notes get 'continue', the last gets
    'stop'. Then merge_tied_notes collapses them as if they were ties.
    ...
    def synth_tie_type_for(offset, pitch):
        """
        Return synthesized tie_type for a note based on tie_like_spans.
        Returns one of 'start', 'continue', 'stop', or None.
        """
        for start_off, end_off in tie_like_spans:
            # Tolerance for offset matching to handle small rounding.
    ...
    for element in part.flatten().notes:
        if isinstance(element, m21.note.Note):
            ...
            synthetic_tie = synth_tie_type_for(offset, pitch)
            # Natural tie wins if present; otherwise use synthetic.
...
def compute_lbdm_per_part(part, w_pitch=0.25, w_ioi=0.50, w_rest=0.25,
    ...
    notes_list, pi, ioi, rest = extract_lbdm_profiles(
        part, tie_like_spans=tie_like_spans
    )
...
def extract_hairpin_indicators(part, num_frames, division):
    ...
    Hairpins are stretched crescendo/decrescendo wedges. V2.7 treats their
...
def extract_tuplet_indicators(part, num_frames, division):
    ...
    Earlier versions only zeroed LBDM on tuplet-interior notes. V2.7 also
...
def compute_per_part_boundary_signal(chroma_novelty_i, rhythm_novelty_i,
    ...
    """
    B_i(t) with separate chroma/rhythm novelty and stateful suppression.
    ...
    V2.7 semantics:
      - Slur and tuplet masks are hard [start, end) phrase-continuation
        suppression states.
      - Tie-like spans are hard [start, end) no-breath zones.
      - Hairpins attenuate by delta under [start, end) semantics.
      - Boosts are multiplied by the same suppression context, so they
        cannot punch through a continuing slur/tuplet/tie-like span.
    """
    ...
    slur_multiplier = np.where(in_slur_interior_i > 0, 0.0, 1.0)

    if in_tuplet_i is None:
        tuplet_multiplier = 1.0
    else:
        tuplet_multiplier = np.where(in_tuplet_i > 0, 0.0, 1.0)
...
def build_long_note_arrival_grid(notes_list, num_frames, division,
    ...
    """
    Project long-note arrivals to the time grid for candidate selection.

    This is not a standalone boundary signal. It helps choose between two
    nearby candidate peaks when one lands on a short-note/textural change
    and another lands on a plausible long-note phrase endpoint.
    """
    ...
    for note in notes_list:
        ...
        idx = int(round(float(note['offset']) * division))
        if 0 <= idx < num_frames:
...
def detect_boundaries(B, time_grid, peak_height=0.3, peak_distance_frames=20,
                      ...
                      forbidden_mask=None):
    """
    Detect boundaries with optional phrase-aware candidate selection.

    V2.7 behavior:
      1. Collect lower-threshold local maxima from B.
      2. Partition nearby candidates into exclusion windows.
      3. Choose the candidate with the highest selection_score in each
         window. If no selection_score is provided, B itself is used.

    forbidden_mask can hard-remove candidates inside slur/tie/tuplet zones.
    """
    if candidate_height is None:
        candidate_height = peak_height
    score = selection_score if selection_score is not None else B

    peaks, _ = find_peaks(B, height=candidate_height)
...
def add_breath_marks_per_part(original_score, boundaries_per_part,
    ...
    for i, part in enumerate(parts):
        ...
        for bt in boundaries:
            # Safety invariant: never place a breath mark in a tie-like
            # same-pitch slur span.  The span is one sustained event.
            if any(start - 0.01 <= bt < end - 0.01
...
def write_summary_per_part(boundaries_per_part, part_names, activity_levels,
    ...
    summary = {
        'parameters': {
            ...
            'boundary_detection': 'candidate_selection_v2_7',
        },
        'tacet_parts': [
...
def run_sectioning(input_path, output_dir='./output',
    ...
    for i in range(len(parts)):
        ...
        print(f"       part {i:2d} {part_names[i]:25s} "
              ...
              f"tuplets={n_tuplets} struct_events={n_struct}{marker}")
    ...
    for i in range(len(parts)):
        ...
        forbidden = np.maximum.reduce([in_slur_i, in_tuplet_i, in_tie_i])
        selection_score = (
            B_i +
            selection_lbdm_weight * lbdm_n +
            selection_long_weight * normalize_signal(long_note_grids[i]) +
...
def explain(result, part_idx, time_qn=None, measure=None, beat=1.0,
    ...
    in_marker = max(in_slur, in_hp, in_tuplet)
    ...
    if in_slur > 0:
        print("  -> hard slur suppression: multiplier = 0.00")
    if in_tuplet > 0:
        print("  -> hard tuplet [start,end) suppression: multiplier = 0.00")
    if in_slur <= 0 and in_tuplet <= 0 and in_hp > 0:
    ...
    if in_slur > 0 or in_tie_like > 0 or in_tuplet > 0:
        effective_mult = 0.0
    else:
        effective_mult = 1.0 - delta * in_hp
    print(f"  (B_raw + boost) * effective suppression multiplier")
...
def main():
    parser = argparse.ArgumentParser(
        description="V2.7 sectioning pipeline with chroma/rhythm self-SSM, "
    ...
    parser.add_argument('--selection_long_weight', type=float, default=0.25)
