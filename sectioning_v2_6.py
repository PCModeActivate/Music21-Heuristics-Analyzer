...
V2.6 of the sectioning pipeline. Changes from v2.5:

0. Set the slur/tie to be [start, end)

1. Nested slur endpoints no longer relax suppression.
   An inner slur endpoint is ignored as boundary-positive evidence
   if that endpoint is still inside an outer slur.

2. Slur interior is now hard suppression.
   I did not leave it as a 0.5 attenuation. If the phrase slur is active,
   do not section there.

3. Tie-like same-pitch slurs are hard no-breath zones.
   They are still passed to LBDM as tie-like spans, but now the final
   boundary signal is also zeroed over the tie-like span.

4. Tie-like classification is no longer endpoint-only.
   It scans all note/chord onsets between the slur start/end offsets.
   So F-G-E-F is not falsely classified as a same-pitch tie just because
   the endpoints are both F.

5. Slur endpoint boosts are now only for effective endpoints.
   An endpoint is effective only if it is not strictly inside another slur.

6. Boosts no longer punch through suppression.
   The formula is now conceptually:

       B_i = (B_raw + boosts) * suppression_multiplier

   rather than:

       B_i = B_raw * suppression_multiplier + boosts
...
def extract_slur_indicators(part, num_frames, division,
    ...
    Extract slur info for a part. Returns a dict with:
      ...
      in_tie_like:       shape (num_frames,), hard no-breath mask for
                         same-pitch tie-like slurs, inclusive of their
                         endpoints.
    ...
    for span_idx, (start_off, end_off, slur) in enumerate(spans):
        ...
        # Interior of every slur suppresses boundaries.  This is
        # what handles nested slurs: an inner endpoint remains suppressed
        # if an outer slur still contains that time.
        # [start, end)
        ...
        if is_tie_like:
            ...
            # Tie-like spans are one sustained musical event.  Do not
            # allow breath marks at internal seams or at the fake slur
            # endpoints produced by the notation.
            # Again, [start, end)
            if end_idx >= start_idx:
...
def compute_per_part_boundary_signal(novelty_i, lbdm_grid_i,
    ...
    """
    B_i(t) with stateful phrase suppression.
    ...
    v2.6 semantics:
      - Slur interior is a hard phrase-suppression state: if t is strictly
        inside any slur, B_i(t) is zero.  This prevents an endpoint of a
        nested slur from relaxing suppression while an outer slur is still
        active.
      - Tie-like spans are a hard no-breath zone, including endpoints.
      - Hairpin interiors still attenuate by delta rather than hard-vetoing.
      - Boosts are attenuated/vetoed with the evidence they modify; they
        can no longer punch through slur suppression by being added after
        suppression.
    """
    ...
    # Hairpins are phrase-context evidence but not as definitive as slurs.
    hairpin_multiplier = 1.0 - delta * np.clip(in_hairpin_i, 0.0, 1.0)

    # Slur interior means the phrase is still ongoing.  Do not section.
    slur_multiplier = np.where(in_slur_interior_i > 0, 0.0, 1.0)

    B_i = (B_raw + boosts) * hairpin_multiplier * slur_multiplier
...
def run_sectioning(input_path, output_dir='./output',
    ...
    print(f"[9/10] Per-part B_i(t) "
          f"(alpha={alpha}, beta={beta}, hard slur suppression, "
          f"hairpin_delta={delta}, gamma={gamma}, zeta={zeta})")
