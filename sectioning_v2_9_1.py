#!/usr/bin/env python
"""
sectioning_v2_9_1.py

V2.9.1 of the sectioning pipeline. Changes from v2.9:

1. Candidate-selection diagnostics are now stored and explainable.
   V2.9 selected boundaries with candidate_signal, selection_score, and
   acceptance_score, but explain() mostly reported the final B_i frame.  V2.9.1
   stores per-part candidate diagnostics and explain() prints candidate/selection
   values around the inspected point.

2. Rest-boundary cleanup is added before annotation and summary output.
   A selected boundary that lands at/inside a rest is usually redundant as a
   breath mark.  V2.9.1 rejects these rest-boundary targets by default instead
   of silently attaching them to the previous note.  Rests are still allowed to
   contribute context through following-gap support, but they are not treated as
   phrase material.

3. Breath-target resolution and validation are centralized.
   A boundary is resolved to a concrete target note before annotation.  The
   validator rejects targets inside tie-like/natural tie spans, targets with
   outgoing natural ties, and rest-boundary targets.  The resolved target records
   are saved in result['boundary_target_info'] for diagnostics.

4. Short-phrase filtering now uses sounding material, not elapsed time.
   V2.9 counted elapsed seconds between boundaries.  V2.9.1 computes sounding
   quarter notes/seconds and note count from note events only, so trailing or
   interior rests do not make a tiny phrase look musically substantial.

5. V2.9 musical features are otherwise preserved.
   Typed structural-event features, cross-rhythm support, natural-tie masking,
   and continuation penalties are unchanged except for the target validation
   cleanup above.

Usage with caching:
    from sectioning_v2_9_1 import run_sectioning, explain, save_result, load_result
    import os
