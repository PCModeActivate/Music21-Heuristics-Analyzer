...
V2.8 of the sectioning pipeline. Changes from v2.7:

1. Natural ties are now first-class sustained-event constraints.
   Ordinary MusicXML ties are treated as one sustained musical event under
   breath-mark semantics.  A breath after the first/internal segment of a
   tied chain is invalid, so natural ties create a hard no-breath mask on
   [first_tied_segment_onset, final_tied_segment_onset).  A breath after
   the final tied segment remains allowed.

2. Tied-chain LBDM and long-note evidence are anchored at the final tied
   segment, not the first tied segment.
   V2.7 merged tied notes for LBDM but projected the merged event back to
   the chain start.  That could reward exactly the place where breathing is
   impossible.  V2.8 stores a boundary_offset on merged tied events and
   projects LBDM/long-note support to that breath-eligible endpoint.

3. Slurs, hairpins, and tuplets are no longer universal hard rejects.
   The only hard no-breath constraints are true sustained-note identity
   cases: natural ties and same-pitch tie-like slurs.  Slurs, hairpins,
   and tuplets are treated as continuation context by penalties/attenuation,
   not absolute law.  This lets the model handle unusually long slurs or
   musically justified breaks while still discouraging ordinary mid-span
   fragmentation.

4. Candidate collection is more note-aware.
   V2.7 only used long-note arrivals as a tie-breaker among frame peaks.
   V2.8 also lets note-anchored breath-target support seed candidates,
   so a plausible long-note endpoint can compete even when the raw SSM
   frame peak is slightly elsewhere.

5. Candidate selection now includes continuation penalties.
   The phrase-oriented selection score rewards LBDM, long-note arrivals,
   following gaps, and cross-rhythm support, while penalizing slur,
   hairpin, and tuplet continuation context.  Natural ties and tie-like
   same-pitch slurs remain hard-invalid.

6. Diagnostics and summaries include natural-tie information.
   The result dict stores natural_tie_info, and explain() reports the
   natural-tie mask alongside slur, hairpin, tuplet, and tie-like context.
...
def write_summary_per_part(boundaries_per_part, part_names, activity_levels,
    ...
    summary = {
        'parameters': {
            ...
            'boundary_detection': 'candidate_selection_v2_8',
        },
        'tacet_parts': [
...
def main():
    parser = argparse.ArgumentParser(
        description="V2.8 sectioning pipeline with chroma/rhythm self-SSM, "
