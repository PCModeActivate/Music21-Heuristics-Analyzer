#!/usr/bin/env python
"""
sectioning_v2_10_E0c.py

Changes from v2.10-E0b-patch -- E0c (2026-07-15): 
...
def compute_per_part_boundary_signal(chroma_novelty_i, rhythm_novelty_i,
    ...
    if expressive_release_i is not None:
...
# E0b (2026-07-15): onset-landed boundaries are re-anchored to the
...
def release_anchor_boundaries(bdys, part_notes, long_grid, gap_grid,
    ...
    for bt in sorted(float(x) for x in arr):
        ...
        if reason is not None and abs(new_bt - bt) > eps:
...
# F1b (2026-08-07): corrections to the F1 channel. See the patch
...
def add_breath_marks_per_part(original_score, boundaries_per_part,
    ...
    for i, part in enumerate(parts):
        ...
        # E0 (2026-07-10): boundary times are RELEASE instants; build the
...
def run_sectioning(input_path, output_dir='./output',
    ...
    expressive_release_grids, _f1_records = _f1_build_release_grids(
    ...
    for i in range(len(parts)):
        ...
        # E0b: re-anchor onset-landed boundaries BEFORE validation, so
...
def main():
    ...
    parser.add_argument('--f1b_no_slur_deferral', action='store_true',
